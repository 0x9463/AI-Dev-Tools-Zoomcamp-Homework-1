...
@transaction.atomic
def reject_submission(*, household, submission_pk, user, reason):
    if not user.is_active or household.admin_id != user.pk or not is_administrator(user):
        raise PermissionDenied
    reason = reason.strip()
    if not reason:
        raise ValidationError("A written rejection reason is required.")
    now = timezone.now()
    claimed = CompletionSubmission.objects.filter(
        pk=submission_pk, task__household=household, task__status=Task.Status.AWAITING_APPROVAL,
        status=CompletionSubmission.Status.PENDING, reviewed_at__isnull=True, reviewer__isnull=True,
    ).update(status=CompletionSubmission.Status.REJECTED, reviewer=user, reviewed_at=now, rejection_reason=reason)
    if not claimed:
        raise ValidationError("Only unreviewed submissions for tasks Awaiting approval can be rejected.")
    submission = CompletionSubmission.objects.select_related("task").get(pk=submission_pk)
    changed = Task.objects.filter(pk=submission.task_id, status=Task.Status.AWAITING_APPROVAL).update(
        status=Task.Status.PENDING, updated_at=now,
    )
    if not changed:
        raise ValidationError("This task is no longer Awaiting approval.")
    TaskHistory.objects.create(task=submission.task, actor=user, event_type="completion_rejected")
    return submission
